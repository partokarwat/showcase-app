...
class FetchAllCoinsUseCaseTest {
    ...
    @Test
    fun `invoke() should fetch coins, update timestamp, and insert or update coins in database`() =
        runTest {
            // Given
    ...
    @Test
    fun `invoke() should delete coins that are no longer provided by the API`() =
        runTest {
            // Given
    ...
    @Test
    fun `invoke() should throw an exception if there is an error from conversion rate repository`() =
        runTest {
            // Given
